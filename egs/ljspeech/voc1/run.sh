...
dumpdir=/opt/storage/kurofune/parallel-wave-gan/data/lj-16000 # directory to dump features
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    local/data_prep.sh \
        ...
        --dev_set "${dev_set}" \
        --eval_set "${eval_set}" \
        "${download_dir}/LJSpeech-1.1-16000" data
fi
...
if [ -z "${tag}" ]; then
    expdir="/opt/storage/kurofune/parallel-wave-gan/exp/${train_set}_ljspeech_$(basename "${conf}" .yaml)"
else
    expdir="/opt/storage/kurofune/parallel-wave-gan/exp/${train_set}_ljspeech_${tag}"
fi
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    if [ "${n_gpus}" -gt 1 ]; then
        ...
        train="python3 -m parallel_wavegan.bin.train"
    fi
